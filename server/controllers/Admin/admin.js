        ...
        bcrypt.compare(
          ...
          (err, response) => {
            if (response) {
              ...
              let query1 = `SELECT * FROM tblLog WHERE OperId = '${results[0].AuthID}' AND Status ='A'`;
              db.query(query1, (err, result) => {
                if (!err) {
                  if (result.length === 0) {
                    setLoginCredentials(results[0].AuthID);
					...
					res.status(200).json({status: 201, data: "User is already login"});
				  }
                }else{
					console.log(err);
				}
...
const setLoginCredentials = (OperID) => {
  let LoginTime = moment(new Date()).format('YYYY-MM-DD hh:mm:ss');
  let query = 'INSERT INTO tblLog (OperId, LoginTime) VALUES(?, ?)';
  db.query(query, [OperID, LoginTime], (err, result) => {
    if (!err) {
      return result;
    } else {
      console.log(err);
...
exports.Logout = (req, res) => {
  let OperId = req.body.OperId;
  let LogoutTime = moment(new Date()).format('YYYY-MM-DD hh:mm:ss');
  let Status = 'I';
  let query = `UPDATE tblLog SET LogoutTime = ?, Status = ? WHERE OperId = '${OperId}' AND Status='A'`;
  db.query(query, [LogoutTime, Status], (err, result) => {
    if (!err) {
